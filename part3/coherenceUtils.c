...
enum state determineState(cache_t* cache, uint32_t address) {
	...
	uint8_t valid = getValid(cache, blockInfo->blockNumber);
	uint8_t dirty = getDirty(cache, blockInfo->blockNumber);
	uint8_t shared = getShared(cache, blockInfo->blockNumber);
...
void updateState(cache_t* cache, uint32_t address, enum state otherState) {
	/* Your Code Here. */
    evictionInfo_t* blockInfo = findEviction(cache, address);
    uint32_t blockNumber = blockInfo->blockNumber;
        ...
        case MODIFIED:      // DONE
            switch(currState) {
                case MODIFIED:
                    ...
                    decrementLRU(cache, tag, index, oldLRU);
                    break;
                case OWNED: // DONE
                    setState(cache, blockNumber, INVALID);
                    decrementLRU(cache, tag, index, oldLRU);
